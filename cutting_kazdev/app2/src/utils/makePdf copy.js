...
export function makePdf(maps, parts) {
    ...
    parts.forEach((part, index) => {
        ...
        const rowData = [ // [22, 40, 18, 40, 18, 18, 74, 236],
            svg_text(index + 1, 18, h),
            {
                svg: renderToStaticMarkup(svgheight)
            },
            svg_text(top_thick_grinding ? top_thick_grinding : "-", 18, h),
            {
                svg: renderToStaticMarkup(svgWidth)
            },
            svg_text(left_thick_grinding ? left_thick_grinding : "-", 18, h),
            svg_text(part.ids.length, 18, h),
            svg_text(obj.label, 72, h),
            svg_text(discription, 228, h)
        ];
    ...
    content = [
        ...
        },
        {
            table: {
                headerRows: 1,
                widths: [20, 40, 20, 40, 20, 20, 74, 230],
                body: pdfData,
                fontSize: 10,
                pageBreak: 'after',
            },
    ...
    maps.forEach((map, index) => {
        const svg_map = renderToStaticMarkup(map.svg);
...
const svg_text = (text, w, h, size) => {
    const svg = (
        <svg width={w} height={h} xmlns="http://www.w3.org/2000/svg">
            <text x="0" y="0" alignment-baseline="before-edge" font-size={size ? size : 10} fill="black">
                {text}
            </text>
        </svg>
    );
